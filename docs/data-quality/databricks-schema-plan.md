...
| `table_count` | BIGINT | Count of tables processed in the run. |
| `column_count` | BIGINT | Count of profiled columns. |
| `dq_score_profiling` / `dq_score_testing` | DOUBLE | Aggregated scores for dashboards. |
| `profile_mode` / `profile_version` | STRING | Metadata describing notebook configuration and version. |
...
Stores profiling run summaries.
...
| `table_count` | BIGINT | Number of tables covered in the run. |
| `column_count` | BIGINT | Number of columns profiled. |
| `anomaly_count` | INT | Count of detected issues. |
| `dq_score_profiling` / `dq_score_testing` | DOUBLE | Aggregated scores for dashboards. |
| `profile_mode` / `profile_version` | STRING | Notebook configuration + version metadata. |
...
Legacy anomaly table maintained for backward compatibility. Future UI paths should move to `dq_profile_anomaly_results`.
...
### 6a. `dq_profile_anomaly_types`

Reference table for anomaly categories surfaced across the UI.

| Column | Type | Notes |
| --- | --- | --- |
| `anomaly_type_id` | STRING | Primary key (stable identifier). |
| `name` | STRING | Display name. |
| `category` | STRING | Grouping (`pii`, `hygiene`, etc.). |
| `default_severity` | STRING | Severity when no override exists. |
| `default_likelihood` | STRING | Likelihood label used in hygiene UI. |
| `description` | STRING | Rich tooltip/description. |
| `created_at` / `updated_at` | TIMESTAMP | Audit metadata. |

### 6b. `dq_profile_results`

Canonical per-column metrics emitted from the notebook (superset of the legacy `dq_profile_columns`).

| Column | Type | Notes |
| --- | --- | --- |
| `result_id` | STRING | Primary key. |
| `profile_run_id` | STRING | FK → `dq_profiles`. |
| `table_id` / `column_id` | STRING | Optional identifiers aligned with catalog metadata. |
| `schema_name` / `table_name` / `column_name` | STRING | Physical identifiers. |
| `data_type` / `general_type` | STRING | Spark type and normalized TestGen type. |
| `record_count` | BIGINT | Total table rows scanned. |
| `null_count` / `distinct_count` | BIGINT | Key metrics. |
| `min_value` / `max_value` / `avg_value` / `stddev_value` | STRING/DOUBLE | Summary stats stored as text where necessary. |
| `percentiles_json` | STRING | JSON payload of percentile approximations (p25/p50/p75/p95). |
| `top_values_json` | STRING | JSON list of top values for quick retrieval. |
| `metrics_json` | STRING | Extension point for additional notebook metrics. |
| `generated_at` | TIMESTAMP | Defaults to `current_timestamp()`. |

### 6c. `dq_profile_anomaly_results`

Structured anomaly rows keyed by `profile_run_id` and catalog identifiers.

| Column | Type | Notes |
| --- | --- | --- |
| `anomaly_result_id` | STRING | Primary key. |
| `profile_run_id` | STRING | FK → `dq_profiles`. |
| `table_id` / `column_id` | STRING | Optional references to catalog metadata tables. |
| `table_name` / `column_name` | STRING | Physical identifiers for quick lookup. |
| `anomaly_type_id` | STRING | FK → `dq_profile_anomaly_types`. |
| `severity` / `likelihood` | STRING | Display-friendly labels. |
| `detail` | STRING | Human-readable explanation. |
| `pii_risk` | STRING | Optional classification reused on hygiene pages. |
| `dq_dimension` | STRING | Dimension for scoring rollups. |
| `detected_at` | TIMESTAMP | Timestamp generated by notebook. |
| `dismissed` / `dismissed_by` / `dismissed_at` | BOOLEAN/STRING/TIMESTAMP | Workflow support for acknowledging issues. |

### 6d. `dq_profile_operations`

Audit log capturing each persistence step executed by the notebook writers.

| Column | Type | Notes |
| --- | --- | --- |
| `operation_id` | STRING | Primary key (UUID). |
| `profile_run_id` | STRING | FK → `dq_profiles`. |
| `target_table` | STRING | Name of the metadata table being written. |
| `rows_written` | BIGINT | Number of rows affected. |
| `duration_ms` | BIGINT | Execution time for the write block. |
| `status` | STRING | `success`, `failed`, `skipped`. |
| `error_payload` | STRING | Serialized error/context when failures occur. |
| `started_at` / `completed_at` | TIMESTAMP | Timing metadata. |

### 6e. `dq_profile_columns`

Materialized column-level metrics retained for compatibility. New work should target `dq_profile_results`, but this table remains for downstream consumers that already reference it.
...
| `general_type` | STRING | Normalized TestGen type (`A`, `N`, `D`, `B`, `X`). |
| `ordinal_position` | INT | Column order inside the table. |
| `row_count` | BIGINT | Total table rows seen during profiling. |
| `null_count` / `non_null_count` | BIGINT | Convenience metrics. |
| `distinct_count` | BIGINT | `approx_count_distinct` result. |
| `min_value` / `max_value` | STRING | Stringified extrema. |
| `avg_value` / `stddev_value` | DOUBLE | Numeric summaries. |
| `median_value` / `p95_value` | DOUBLE | Percentile approximations. |
| `true_count` / `false_count` | BIGINT | Boolean counts. |
| `min_length` / `max_length` / `avg_length` | DOUBLE | String length summaries. |
| `non_ascii_ratio` | DOUBLE | Ratio of rows containing non-ASCII characters. |
| `min_date` / `max_date` | DATE | Date extrema. |
| `date_span_days` | INT | `datediff(max_date, min_date)`. |
| `metrics_json` | STRING | JSON blob for extensibility (e.g., pattern counts). |
| `generated_at` | TIMESTAMP | Default `current_timestamp()`. |

Storage guidance remains unchanged: partition by `profile_run_id`, Z-Order on (`profile_run_id`, `table_name`, `column_name`), and write via `MERGE`.

### 6f. `dq_profile_column_values`
...
| `bucket_lower_bound` / `bucket_upper_bound` | DOUBLE | Numeric bucket edges. |
| `generated_at` | TIMESTAMP | Default `current_timestamp()`. |
...
### 6g. `dq_data_table_chars`

Table-level catalog metadata enriched by the profiling notebook.

| Column | Type | Notes |
| --- | --- | --- |
| `table_id` | STRING | Primary key (matches `dq_tables.table_id`). |
| `table_group_id` | STRING | FK for grouping/filtering. |
| `schema_name` / `table_name` | STRING | Physical identifiers. |
| `record_count` / `column_count` / `data_point_count` | BIGINT/INT | Snapshot metrics. |
| `critical_data_element` | BOOLEAN | Flag used in hygiene dashboards. |
| `data_source` / `source_system` / `source_process` | STRING | Tag metadata surfaced in the UI. |
| `business_domain` / `stakeholder_group` / `transform_level` / `data_product` | STRING | Additional tags. |
| `dq_score_profiling` / `dq_score_testing` | DOUBLE | Aggregated table scores. |
| `last_complete_profile_run_id` | STRING | FK to the latest completed run for this table. |
| `latest_anomaly_ct` | INT | Count of anomalies from the latest run. |
| `latest_run_completed_at` | TIMESTAMP | Timestamp of most recent completed run. |
| `created_at` / `updated_at` | TIMESTAMP | Audit fields. |

### 6h. `dq_data_column_chars`

Column-level catalog metadata synced from data definitions plus profiling output.

| Column | Type | Notes |
| --- | --- | --- |
| `column_id` | STRING | Primary key (matches internal column GUID). |
| `table_id` | STRING | FK → `dq_data_table_chars`. |
| `schema_name` / `table_name` / `column_name` | STRING | Physical identifiers. |
| `data_type` / `functional_data_type` | STRING | Technical + semantic typing. |
| `critical_data_element` | BOOLEAN | Flag inherited from catalog. |
| `pii_risk` | STRING | Notebook-detected PII risk classification. |
| `dq_dimension` | STRING | Dimension for scoring rollups. |
| `tags_json` | STRING | Serialized tag map (business owners, lineage, etc.). |
| `dq_score_profiling` / `dq_score_testing` | DOUBLE | Column level scores. |
| `last_complete_profile_run_id` | STRING | FK to latest run touching the column. |
| `latest_anomaly_ct` | INT | Anomaly count for the latest run. |
| `latest_run_completed_at` | TIMESTAMP | Timestamp of latest run. |
| `created_at` / `updated_at` | TIMESTAMP | Audit fields. |
