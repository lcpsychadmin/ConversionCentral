...
import { ConstructedField, ConstructedData, batchSaveConstructedData, createConstructedData, updateConstructedData, deleteConstructedData, ValidationError } from '../../services/constructedDataService';
...
const ConstructedDataGridReactGrid: React.FC<Props> = ({ constructedTableId, fields, rows, onDataChange }) => {
  ...
  const updateCellLocal = useCallback((rowIndex: number, field: string, value: any) => {
    setLocalRows(prev => {
  ...
  const saveRow = useCallback(async (row: ConstructedData) => {
    try {
      ...
      if (String(row.id).startsWith(emptyRowIdPrefix)) {
        ...
        setLocalRows(prev => prev.map(r => (r.id === row.id ? created : r)));
        toast.showToast('Row created', 'success');
        return { success: true, row: created };
      } else {
  ...
  const onCellBlur = useCallback(async (rowIndex: number, field: string) => {
          ...
          <tbody>
            {rowsWithBlank.map((r, rowIndex) => (
              <tr key={r.id} style={{ background: r.id.toString().startsWith(emptyRowIdPrefix) ? alpha(theme.palette.success.main, 0.05) : 'transparent' }}>
                ...
                {fields.map(f => (
                  <td key={`${r.id}-${f.name}`} style={{ padding: 6, borderRight: `1px solid ${alpha(theme.palette.divider, 0.3)}` }}>
                    <div
                      contentEditable
                      ...
                      onBlur={(e) => {
                        ...
                        void onCellBlur(rowIndex, f.name);
                      }}
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          e.preventDefault();
