...
const InventoryPage = () => {
    ...
    const systemsErrorMessage = systemsError
        ...
        : null;
    return (_jsxs(Box, { children: [_jsxs(Stack, { direction: "row", justifyContent: "space-between", alignItems: "center", mb: 3, children: [_jsxs("div", { children: [_jsx(Typography, { variant: "h4", gutterBottom: true, children: "Inventory" }), _jsx(Typography, { variant: "body2", color: "text.secondary", children: "Manage data object inventory, process area alignment, and system ownership." })] }), canManage && (_jsx(Button, { variant: "contained", onClick: handleCreateClick, disabled: busy, children: "New Data Object" }))] }), _jsx(Paper, { elevation: 1, sx: { p: 2, mb: 3 }, children: _jsx(DataObjectTable, { data: rows, loading: dataObjectsLoading, selectedId: selected?.id ?? null, canManage: canManage, onSelect: handleSelect, onEdit: canManage ? handleEdit : undefined, onDelete: canManage ? handleDelete : undefined }) }), dataObjectsErrorMessage && (_jsx(Alert, { severity: "error", sx: { mb: 3 }, children: dataObjectsErrorMessage })), processAreasErrorMessage && (_jsx(Alert, { severity: "error", sx: { mb: 3 }, children: processAreasErrorMessage })), systemsErrorMessage && (_jsx(Alert, { severity: "error", sx: { mb: 3 }, children: systemsErrorMessage })), canManage && noProcessAreasAvailable && (_jsx(Alert, { severity: "info", sx: { mb: 3 }, children: "No process areas are available yet. Create a process area to enable new data objects." })), selected && (_jsx(Paper, { elevation: 1, sx: { p: 3 }, children: _jsxs(Grid, { container: true, spacing: 2, children: [_jsxs(Grid, { item: true, xs: 12, md: 6, children: [_jsx(Typography, { variant: "h6", gutterBottom: true, children: "Details" }), _jsxs(Stack, { spacing: 1, children: [_jsx(DetailLine, { label: "Name", value: selected.name }), _jsx(DetailLine, { label: "Domain", value: selected.processAreaName ?? 'Unassigned' }), _jsx(DetailLine, { label: "Description", value: selected.description ?? '—' })] })] }), _jsxs(Grid, { item: true, xs: 12, md: 6, children: [_jsx(Typography, { variant: "h6", gutterBottom: true, children: "Status" }), _jsx(Chip, { label: selected.status, color: selected.status === 'active' ? 'success' : 'default' }), _jsx(Divider, { sx: { my: 1.5 } }), _jsx(Typography, { variant: "subtitle2", color: "text.secondary", gutterBottom: true, children: "Systems" }), (selected.systems?.length ?? 0) === 0 ? (_jsx(Typography, { variant: "body2", color: "text.secondary", children: "No systems assigned." })) : (_jsx(Stack, { direction: "row", spacing: 1, flexWrap: "wrap", useFlexGap: true, children: selected.systems?.map((system) => (_jsx(Chip, { label: system.name, size: "small" }, system.id))) }))] })] }) })), canManage && (_jsx(DataObjectForm, { open: formOpen, title: formMode === 'create' ? 'Create Data Object' : 'Edit Data Object', initialValues: formMode === 'edit' ? selected : null, loading: busy, onClose: handleFormClose, onSubmit: handleFormSubmit, processAreas: processAreas, systems: systems })), _jsx(ConfirmDialog, { open: confirmOpen, title: "Delete Data Object", description: `Are you sure you want to delete "${selected?.name ?? ''}"? This action cannot be undone.`, confirmLabel: "Delete", onClose: () => setConfirmOpen(false), onConfirm: handleConfirmDelete })] }));
};
