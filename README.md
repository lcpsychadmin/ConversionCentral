...
- `POST /roles` – manage reusable RBAC roles such as Process Area Owner or Data Custodian.
- `POST /users` – register users who participate in governance workflows (name, email, status).
- `POST /process-area-role-assignments` – bind a user and role to a process area (path retained for backward-compatible routing), optionally capturing who granted access.
